...
public class Zero extends Attacker
{
	...
	public void move()
	{
		...
		if(permaStop)
		{
			...
			if(img.getTranslateX() < closest - window.getWidth() * .04008333) //img.getTranslateX() < enemy.getTranslateX() - 100)
			{
			...
			{
				/*if(closestEnemy.size() > 0)
					if(closestEnemy.get(enemyKilled).getX() - img.getTranslateX() < window.getWidth() * .10)
					{
						try
						{
							createBullet(img.getTranslateX(), img.getTranslateY(), closestEnemy.get(enemyKilled));
						} catch (Exception e1)
						{
			...
			}else
			{
				...
				if(imgEnemy.getTranslateX() > closest + window.getWidth() * .03908333) //img.getTranslateX() < enemy.getTranslateX() - 100)
				{
					TranslateTransition t = new TranslateTransition();
					t.setDuration(Duration.millis(1));
					t.setNode(imgEnemy);
