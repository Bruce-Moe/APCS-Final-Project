...
public class TroopControl extends GameMenuDemo
{
	...
	public ImageView createTroop(Attacker a, boolean friendly) throws Exception
	{
		...
		if(friendly)
		{
			if(a instanceof One)
			{
				Image m = new Image(new FileInputStream("res/imgs/one.png"), window.getHeight() * .075, window.getHeight() * .075, false, false);
				img = new ImageView(m);
				img.setTranslateX(window.getWidth() * .087333);
				img.setTranslateY(window.getHeight() * .844269231);
				a.setImg(img);
			}
			if(a instanceof Zero)
			{
				Image m = new Image(new FileInputStream("res/imgs/zero.png"), window.getHeight() * .075, window.getHeight() * .075, false, false);
				img = new ImageView(m);
				img.setTranslateX(window.getWidth() * .087333);
				img.setTranslateY(window.getHeight() * .844269231);
				a.setImg(img);
	...
	public void incrementEnemy(ImageView i)
	{
		closestEnemy.add(i);
	}
